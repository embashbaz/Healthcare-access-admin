...
class CoordinateDialog() : DialogFragment(){
    ...
    private fun setDataTo() : GeoPoint{
        if (!lantitudeTl.editText?.text.isNullOrEmpty() && !longitudeTl.editText?.text.isNullOrEmpty() ){

              return  GeoPoint(lantitudeTl.editText?.text.toString().toDouble(), longitudeTl.editText?.text.toString().toDouble())
    ...
    interface CoordinateDialogListener {
        fun onDialogPositiveClick(geoPoint: GeoPoint)
